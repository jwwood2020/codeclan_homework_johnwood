...
> * **Incidence rates do not vary significantly between NHS Borders and the rest of Scotland. **
...
Three of these are skin cancers. 
```{r echo = FALSE}
summary_counts_borders %>% 
  filter(sex == "All" & 
...
top10_rates_borders %>% 
  filter(sex == "All") %>% 
  ggplot() +
  geom_linerange(aes(reorder(cancer_site, crude_rate), ymin = 0, ymax = crude_rate, colour = area), 
                   position = position_dodge(width = 1)) +
  geom_point(aes(x = cancer_site, y = crude_rate, colour = area),
               position = position_dodge(width = 1)) +
  scale_colour_manual(values = c("NHS Borders" = "forestgreen",
                               "Scotland" = "blue")) +
  theme_classic() +
