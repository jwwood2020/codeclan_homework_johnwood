...
```{r}
drink_calories <- select(starbucks, Beverage_category, Beverage, Beverage_prep, Calories)
...
```{r}
drink_calories <- mutate(drink_calories, calorie_diff = 135 - Calories)
...
drinks_grouped <- group_by(drink_calories, Beverage_category)
...
dinks_grouped <- group_by(drink_calories, Beverage_prep)
filter(drinks_grouped, Calories > mean_calories)
```
